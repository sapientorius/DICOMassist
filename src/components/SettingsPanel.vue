      ...
      <div class="space-y-4 p-4">
        ...
        <fieldset class="space-y-2 border-t border-neutral-700 pt-3">
          ...
          <div class="grid grid-cols-2 gap-2 text-[11px]">
            <label class="text-neutral-400">Max images<input type="number" min="1" max="100" :value="analysis.maxImages" class="mt-1 w-full rounded border border-neutral-700 bg-neutral-900 p-1.5 text-neutral-100" @input="updateAnalysis({ maxImages: Number(($event.target as HTMLInputElement).value) })"></label>
            ...
            <label class="text-neutral-400">Refinement rounds<input type="number" min="0" max="5" :value="analysis.maxRefinementRounds" class="mt-1 w-full rounded border border-neutral-700 bg-neutral-900 p-1.5 text-neutral-100" @input="updateAnalysis({ maxRefinementRounds: Number(($event.target as HTMLInputElement).value) })"></label>
          </div>
