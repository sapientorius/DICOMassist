...
<template>
  <aside class="flex h-full w-96 shrink-0 flex-col overflow-hidden border-l border-neutral-700 bg-neutral-900" aria-label="Analysis chat">
    ...
    <div ref="scroll" class="flex-1 space-y-3 overflow-y-auto p-3">
      ...
      <div v-if="pipeline?.budget" class="rounded border border-neutral-800 p-2 text-[11px] text-neutral-400"><p class="font-medium text-neutral-200">Current context workset</p><p>{{ pipeline.activeImageCount }} active / {{ pipeline.archiveImageCount }} archived images · {{ Math.round(pipeline.budget.pixelsPerImage / 1000) }}k pixels/image · ~{{ Math.round(pipeline.budget.estimatedInputTokens / 100) / 10 }}k input tokens</p><p>Context reserve: {{ Math.max(0, pipeline.budget.availableInputTokens - pipeline.budget.estimatedInputTokens).toLocaleString() }} tokens · {{ pipeline.excludedImageCount }} excluded · {{ pipeline.ledgerEntryCount }} ledger entries</p><p v-for="warning in pipeline.budget.warnings" :key="warning" class="text-amber-300">{{ warning }}</p></div>
      ...
      <details v-if="pipeline" class="rounded border border-neutral-800 p-2 text-[10px] text-neutral-500"><summary class="cursor-pointer text-neutral-300">Run comparison log</summary><p>{{ pipeline.comparisonLog.provider }} · plan: {{ pipeline.comparisonLog.planningModel }} · vision: {{ pipeline.comparisonLog.visionModel }}</p><p>Profile: {{ pipeline.comparisonLog.settings.profile }} · {{ pipeline.comparisonLog.settings.contextWindowTokens }} context tokens · {{ pipeline.comparisonLog.settings.responseTokenBudget }} response tokens</p><p v-for="round in pipeline.comparisonLog.rounds" :key="round.round">Round {{ round.round + 1 }}: {{ round.imageCount }} active / {{ round.archivedImageCount ?? round.imageCount }} archived · ~{{ Math.round((round.estimatedInputTokens ?? 0) / 100) / 10 }}k tokens · {{ round.ledgerEntryCount ?? 0 }} ledger entries · {{ round.excludedImageCount ?? 0 }} excluded · {{ round.durationMs ?? 0 }}ms{{ round.requestedAdditionalImages ? ' · requested targeted images' : '' }}</p></details>
