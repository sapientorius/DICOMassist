...
import { buildAnalysisSystemPrompt } from './PromptBuilder';

describe('analysis refinement prompt', () => {
  it('requires every retrieval round to produce a standalone user-facing synthesis', () => {
    ...
    expect(prompt).toContain('## USER-FACING, STANDALONE SYNTHESIS');
    expect(prompt).toContain('ONLY analysis the user sees');
    expect(prompt).toContain('Do NOT refer to a "previous response", "previous round", "earlier assessment", "prior evaluation",');
    expect(prompt).toContain('not "Additional images confirm the prior finding".');
  });
  ...
  it('requires a concrete image request for sampling-based limitations while capacity remains', () => {
    ...
    expect(prompt).toContain('The client retrieves the requested images locally');
